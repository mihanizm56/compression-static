...
module.exports.compressFiles = ({ dirs, options }) => {
  // console.log('get in compressFiles');
  // console.log(dirsToCompress, options);

  const compressConfigs = [];

  if (!dirs || !options) {
    console.log('no valid i/o were provided');
    return;
  }
  ...
  try {
    ...
    console.log('no valid i/o were provided', error);
